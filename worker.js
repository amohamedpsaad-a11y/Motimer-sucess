...
//    generated for that checkout.
// 3. It serves a simple HTML page with one button: "Activate Motimer".
//    Clicking it sends the key straight into the extension via
//    chrome.runtime.sendMessage (allowed because of "externally_connectable"
//    in manifest.json) — no copy/paste needed.
//
...
//      checkouts:read, license_keys:read).
//   3. Worker -> Settings -> Variables -> add a plain variable
//      POLAR_ORG_ID = 0e4771fb-1ab4-41d6-a759-4362e4643bc7
//   4. Worker -> Triggers -> add a custom domain / route: motimer.online/success*
//      (motimer.online is already on Cloudflare-manageable DNS via Namecheap,
//      or you can point a CNAME at the workers.dev address instead).
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      const licenseKey = await findLicenseKeyForCustomer(env, checkout.customer_id);
      if (!licenseKey) {
      ...
      return htmlResponse(renderSuccess(licenseKey), 200);
    } catch (err) {
...
function renderSuccess(licenseKey) {
  ...
  <body>
    ...
    <script>
      ...
      btn.addEventListener("click", () => {
        ...
        chrome.runtime.sendMessage(EXTENSION_ID, { type: "MOTIMER_ACTIVATE_LICENSE", key: LICENSE_KEY }, (response) => {
